namespace mos6502.Tests;

public class AslTests
{
  [Theory]
  [InlineData(0x01, 0x02, false, false, false)]
  [InlineData(0x40, 0x80, false, false, true)]
  [InlineData(0x80, 0x00, true, true, false)]
  [InlineData(0x81, 0x02, true, false, false)]
  [InlineData(0xFF, 0xFE, true, false, true)]
  [InlineData(0x00, 0x00, false, true, false)]
  public void Asl_Accumulator_ShiftsLeftAndUpdatesFlags(
    u8 initial,
    u8 expected,
    bool carry,
    bool zero,
    bool negative)
  {
    var bus = new Bus();
    var cpu = new Emulator(bus);
    cpu.LDA_immediate(initial);

    var cycles = cpu.ASL_accumulator();

    cpu.A.Should().Be(expected);
    cpu.Status.HasFlag(Status.Carry).Should().Be(carry);
    cpu.Status.HasFlag(Status.Zero).Should().Be(zero);
    cpu.Status.HasFlag(Status.Negative).Should().Be(negative);
    cycles.Should().Be(2);
  }

  [Fact]
  public void Asl_Accumulator_PreservesUnrelatedFlags()
  {
    var bus = new Bus();
    var cpu = new Emulator(bus);
    cpu.LDA_immediate(0x01);

    cpu.ASL_accumulator();

    cpu.Status.Should().Be(Status.Interrupt);
  }

  [Fact]
  public void Asl_ZeroPage_ReadsModifiesAndWritesMemoryAndReturnsFiveCycles()
  {
    var bus = new Bus();
    bus.WriteByte(0x0042, 0x81);
    var cpu = new Emulator(bus);

    var cycles = cpu.ASL_zero_page(0x42);

    bus.ReadByte(0x0042).Should().Be(0x02);
    cpu.Status.HasFlag(Status.Carry).Should().BeTrue();
    cpu.Status.HasFlag(Status.Zero).Should().BeFalse();
    cpu.Status.HasFlag(Status.Negative).Should().BeFalse();
    cycles.Should().Be(5);
  }

  [Fact]
  public void Asl_ZeroPageX_AppliesXOffsetAndReturnsSixCycles()
  {
    var bus = new Bus();
    bus.WriteByte(0x0084, 0x40);
    var cpu = new Emulator(bus);
    cpu.LDX_immediate(0x04);

    var cycles = cpu.ASL_zero_page_x(0x80);

    bus.ReadByte(0x0084).Should().Be(0x80);
    cpu.Status.HasFlag(Status.Negative).Should().BeTrue();
    cpu.Status.HasFlag(Status.Carry).Should().BeFalse();
    cycles.Should().Be(6);
  }

  [Fact]
  public void Asl_ZeroPageX_WrapsWithinZeroPage()
  {
    var bus = new Bus();
    bus.WriteByte(0x0004, 0x01);
    var cpu = new Emulator(bus);
    cpu.LDX_immediate(0x05);

    var cycles = cpu.ASL_zero_page_x(0xFF);

    bus.ReadByte(0x0004).Should().Be(0x02);
    cycles.Should().Be(6);
  }

  [Fact]
  public void Asl_Absolute_ReadsModifiesAndWritesMemoryAndReturnsSixCycles()
  {
    var bus = new Bus();
    bus.WriteByte(0x2000, 0xC0);
    var cpu = new Emulator(bus);

    var cycles = cpu.ASL_absolute(0x2000);

    bus.ReadByte(0x2000).Should().Be(0x80);
    cpu.Status.HasFlag(Status.Carry).Should().BeTrue();
    cpu.Status.HasFlag(Status.Negative).Should().BeTrue();
    cycles.Should().Be(6);
  }

  [Fact]
  public void Asl_AbsoluteX_WithoutPageCrossing_ReturnsSevenCycles()
  {
    var bus = new Bus();
    bus.WriteByte(0x2005, 0x02);
    var cpu = new Emulator(bus);
    cpu.LDX_immediate(0x05);

    var cycles = cpu.ASL_absolute_x(0x2000);

    bus.ReadByte(0x2005).Should().Be(0x04);
    cycles.Should().Be(7);
  }

  [Fact]
  public void Asl_AbsoluteX_WithPageCrossing_ReturnsSevenCycles()
  {
    var bus = new Bus();
    bus.WriteByte(0x2102, 0x80);
    var cpu = new Emulator(bus);
    cpu.LDX_immediate(0x05);

    var cycles = cpu.ASL_absolute_x(0x20FD);

    bus.ReadByte(0x2102).Should().Be(0x00);
    cpu.Status.HasFlag(Status.Carry).Should().BeTrue();
    cpu.Status.HasFlag(Status.Zero).Should().BeTrue();
    cycles.Should().Be(7);
  }
}
